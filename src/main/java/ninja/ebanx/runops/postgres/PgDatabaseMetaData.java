...
public class PgDatabaseMetaData implements DatabaseMetaData {
    ...
    private final PgTarget pgTarget;

    public PgDatabaseMetaData(RunopsConnection conn) {
        this.connection = conn;
        this.pgTarget = (PgTarget)conn.getTargetConnection();
    }

    private String keywords;

    protected final RunopsConnection connection; // The connection association
            ...
            try {
                ...
                if (!rs.next()) {
                    ...
                    throw new SQLException(
                            "Unable to determine a value for MaxIndexKeys due to missing system catalog data.", "99999");
                }
                indexMaxKeys = rs.getInt(1);
            } finally {
                BlackHole.close(rs);
                BlackHole.close(stmt);
            }
        }
        return indexMaxKeys;
    }
    ...
    protected int getMaxNameLength() throws SQLException {
        if (nameDataLength == 0) {
            ...
            try {
                rs = stmt.executeQuery(sql);
                if (!rs.next()) {
                    throw new SQLException("Unable to find name datatype in the system catalogs.", "99999");
                }
    ...
    public String getURL() {
        return pgTarget.getURL();
    }

    public String getUserName() {
        return pgTarget.getUserName();
    }
    ...
    public String getDriverName() {
        return Driver.Version.DRIVER_NAME + " (PostgreSQL)";
    }
    ...
    public int getDriverMajorVersion() {
        return Driver.Version.MAJOR_VERSION;
    }

    @Override
    public int getDriverMinorVersion() {
        return Driver.Version.MINOR_VERSION;
    }
    ...
    public boolean supportsTransactionIsolationLevel(int level) {
        return switch (level) {
            case Connection.TRANSACTION_READ_UNCOMMITTED, Connection.TRANSACTION_READ_COMMITTED, Connection.TRANSACTION_REPEATABLE_READ, Connection.TRANSACTION_SERIALIZABLE ->
                    true;
            default -> false;
        };
    }
    ...
    public ResultSet getProcedures(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        if (pgTarget.haveMinimumServerVersion(ServerVersion.v11)) {
            sql += " AND p.prokind='p'";
        }
        ...
        if (pgTarget.getHideUnprivilegedObjects()) {
    ...
    public ResultSet getProcedureColumns(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        while (rs.next()) {
            ...
            byte[] specificName =
                    Utils.encodeString(rs.getString("proname") + "_" + rs.getString("oid"));
            ...
            if ("b".equals(returnTypeType) || "d".equals(returnTypeType) || "e".equals(returnTypeType)
                ...
                tuple[5] = Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getSQLType(returnType)));
                tuple[6] = Utils.encodeString(Objects.requireNonNull(pgTarget.getTypeInfo().getPGType(returnType)));
                tuple[7] = null;
                tuple[8] = null;
                tuple[9] = null;
                tuple[10] = null;
                tuple[11] = Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.procedureNullableUnknown));
                tuple[12] = null;
                tuple[17] = Utils.encodeString(Integer.toString(0));
            ...
            for (int i = 0; i < numArgs; i++) {
                ...
                if (argNames != null) {
                    tuple[3] = Utils.encodeString(argNames[i]);
                } else {
                    tuple[3] = Utils.encodeString("$" + (i + 1));
                }
                ...
                tuple[4] = Utils.encodeString(Integer.toString(columnMode));
                ...
                tuple[5] =
                        Utils.encodeString(Integer.toString(
                                castNonNull(pgTarget.getTypeInfo().getSQLType(argOid))));
                tuple[6] = Utils.encodeString(Objects.requireNonNull(pgTarget.getTypeInfo().getPGType(argOid)));
                tuple[7] = null;
                tuple[8] = null;
                tuple[9] = null;
                tuple[10] = null;
                tuple[11] =
                        Utils.encodeString(Integer.toString(DatabaseMetaData.procedureNullableUnknown));
                tuple[12] = null;
                tuple[17] = Utils.encodeString(Integer.toString(i + 1));
                tuple[18] = isnullableUnknown;
                tuple[19] = specificName;
            ...
            if ("c".equals(returnTypeType) || ("p".equals(returnTypeType) && argModesArray != null)) {
                ...
                while (columnrs.next()) {
                    ...
                    tuple[5] = Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getSQLType(columnTypeOid)));
                    tuple[6] = Utils.encodeString(Objects.requireNonNull(pgTarget.getTypeInfo().getPGType(columnTypeOid)));
                    tuple[7] = null;
                    tuple[8] = null;
                    tuple[9] = null;
                    tuple[10] = null;
    ...
    public ResultSet getTableTypes() {
        ...
        for (String type : types) {
            byte[] @Nullable [] tuple = new byte[1][];
            tuple[0] = Utils.encodeString(type);
            v.add(new Tuple(tuple));
        }
    ...
    public ResultSet getColumns(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        //
        if (pgTarget.haveMinimumServerVersion(ServerVersion.v8_4)) {
            sql = "SELECT * FROM (";
        } else {
            sql = "";
        }
        ...
        if (pgTarget.haveMinimumServerVersion(ServerVersion.v10)) {
            sql += "nullif(a.attidentity, '') as attidentity,";
        } else {
            sql += "null as attidentity,";
        }

        if (pgTarget.haveMinimumServerVersion(ServerVersion.v12)) {
        ...
        while (rs.next()) {
            ...
            if ("c".equals(typtype)) {
                ...
                sqlType = pgTarget.getTypeInfo().getSQLType(typeOid);
            }

            tuple[4] = Utils.encodeString(Integer.toString(sqlType));
            String pgType = pgTarget.getTypeInfo().getPGType(typeOid);
            assert pgType != null;
            ...
            if (defval != null && defval.contains("nextval(") ) {
                if ("int4".equals(pgType)) {
                    tuple[5] = Utils.encodeString("serial"); // Type name == serial
                } else if ("int8".equals(pgType)) {
                    tuple[5] = Utils.encodeString("bigserial"); // Type name == bigserial
                } else if ("int2".equals(pgType) && pgTarget.haveMinimumServerVersion(ServerVersion.v9_2)) {
                    tuple[5] = Utils.encodeString("smallserial"); // Type name == smallserial
                }
            }
         ...
         */
                ...
                decimalDigits = pgTarget.getTypeInfo().getScale(baseTypeOid, typeMod);
        /*
         ...
         */
                if ( typtypmod == -1 ) {
                    columnSize = pgTarget.getTypeInfo().getPrecision(baseTypeOid, typeMod);
                } else if (baseTypeOid == Oid.NUMERIC ) {
                    decimalDigits = pgTarget.getTypeInfo().getScale(baseTypeOid, typtypmod);
                    columnSize = pgTarget.getTypeInfo().getPrecision(baseTypeOid, typtypmod);
                } else {
                    columnSize = typtypmod;
                }
            } else {
                decimalDigits = pgTarget.getTypeInfo().getScale(typeOid, typeMod);
                columnSize = pgTarget.getTypeInfo().getPrecision(typeOid, typeMod);
                if ( sqlType != Types.NUMERIC && columnSize == 0 ) {
                    columnSize = pgTarget.getTypeInfo().getDisplaySize(typeOid, typeMod);
                }
            }
            tuple[6] = Utils.encodeString(Integer.toString(columnSize));
            // Give null for an unset scale on Decimal and Numeric columns
            if (((sqlType == Types.NUMERIC) || (sqlType == Types.DECIMAL)) && (typeMod == -1)) {
                tuple[8] = null;
            } else {
                tuple[8] = Utils.encodeString(Integer.toString(decimalDigits));
            }

            // Everything is base 10 unless we override later.
            tuple[9] = Utils.encodeString("10");

            if ("bit".equals(pgType) || "varbit".equals(pgType)) {
                tuple[9] = Utils.encodeString("2");
            }

            tuple[10] = Utils.encodeString(Integer.toString(rs.getBoolean("attnotnull")
            ...
            tuple[16] = Utils.encodeString(String.valueOf(rs.getInt("attnum"))); // ordinal position
            // Is nullable
            tuple[17] = Utils.encodeString(rs.getBoolean("attnotnull") ? "NO" : "YES");
            ...
            tuple[21] = baseTypeOid == 0 // SOURCE_DATA_TYPE
                    ? null
                    : Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getSQLType(baseTypeOid)));

            String autoinc = "NO";
            if (defval != null && defval.contains("nextval(") || identity != null) {
                autoinc = "YES";
            }
            tuple[22] = Utils.encodeString(autoinc); // IS_AUTOINCREMENT

            String generatedcolumn = "NO";
            if (generated != null) {
                generatedcolumn = "YES";
            }
            tuple[23] = Utils.encodeString(generatedcolumn); // IS_GENERATEDCOLUMN
    ...
    public ResultSet getColumnPrivileges(@Nullable String catalog, @Nullable String schema,
        ...
        sql = "SELECT n.nspname,c.relname,r.rolname,c.relacl, "
                + (pgTarget.haveMinimumServerVersion(ServerVersion.v8_4) ? "a.attacl, " : "")
                + " a.attname "
                + " FROM pg_catalog.pg_namespace n, pg_catalog.pg_class c, "
        ...
        while (rs.next()) {
            ...
            for (String permName : permNames) {
                byte[] privilege = Utils.encodeString(permName);
    ...
    public Map<String, Map<String, List<@Nullable String[]>>> parseACL(@Nullable String aclArray,
        ...
        if (aclArray == null) {
            ...
            String perms = pgTarget.haveMinimumServerVersion(ServerVersion.v8_4) ? "arwdDxt" : "arwdxt";
    ...
    public ResultSet getBestRowIdentifier(
        ...
        while (rs.next()) {
            byte[] @Nullable [] tuple = new byte[8][];
            int typeOid = (int) rs.getLong("atttypid");
            int sqlType = pgTarget.getTypeInfo().getSQLType(typeOid);
            int typeMod = rs.getInt("atttypmod");
            int decimalDigits = pgTarget.getTypeInfo().getScale(typeOid, typeMod);
            int columnSize = pgTarget.getTypeInfo().getPrecision(typeOid, typeMod);
            if ( sqlType != Types.NUMERIC && columnSize == 0) {
                columnSize = pgTarget.getTypeInfo().getDisplaySize(typeOid, typeMod);
            }
            tuple[0] = Utils.encodeString(Integer.toString(scope));
            tuple[1] = rs.getBytes("attname");
            tuple[2] =
                    Utils.encodeString(Integer.toString(sqlType));
            tuple[3] = Utils.encodeString(Objects.requireNonNull(pgTarget.getTypeInfo().getPGType(typeOid)));
            tuple[4] = Utils.encodeString(Integer.toString(columnSize));
            tuple[5] = null; // unused
            tuple[6] = Utils.encodeString(Integer.toString(decimalDigits));
            tuple[7] =
                    Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.bestRowNotPseudo));
            v.add(new Tuple(tuple));
        }
        rs.close();
        stmt.close();
    ...
    public ResultSet getVersionColumns(
        ...
        tuple[0] = null;
        tuple[1] = Utils.encodeString("ctid");
        tuple[2] =
                Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getSQLType("tid")));
        tuple[3] = Utils.encodeString("tid");
        tuple[4] = null;
        tuple[5] = null;
        tuple[6] = null;
        tuple[7] =
                Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.versionColumnPseudo));
        v.add(new Tuple(tuple));
    ...
    protected ResultSet getImportedExportedKeys(
        ...
        if (!pgTarget.haveMinimumServerVersion(ServerVersion.v9_0)) {
    ...
    */
    ...
    public ResultSet getTypeInfo() throws SQLException {
        ...
        if (pgTarget.getHideUnprivilegedObjects() && pgTarget.haveMinimumServerVersion(ServerVersion.v9_2)) {
        ...
        // things up a little.
        byte[] bZero = Utils.encodeString("0");
        byte[] b10 = Utils.encodeString("10");
        byte[] bf = Utils.encodeString("f");
        byte[] bt = Utils.encodeString("t");
        byte[] bliteral = Utils.encodeString("'");
        byte[] bNullable =
                Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.typeNullable));
        byte[] bSearchable =
                Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.typeSearchable));

        TypeInfo ti = pgTarget.getTypeInfo();
        ti.cacheSQLTypes();
        ...
        while (rs.next()) {
            ...
            tuple[0] = Utils.encodeString(typname);
            int sqlType = pgTarget.getTypeInfo().getSQLType(typname);
            tuple[1] =
                    Utils.encodeString(Integer.toString(sqlType));
            ...
            tuple[2] = Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getMaximumPrecision(typeOid)));
            ...
            if (pgTarget.getTypeInfo().requiresQuotingSqlType(sqlType)) {
                tuple[3] = bliteral;
                tuple[4] = bliteral;
            }

            tuple[6] = bNullable; // all types can be null
            tuple[7] = pgTarget.getTypeInfo().isCaseSensitive(typeOid) ? bt : bf;
            tuple[8] = bSearchable; // any thing can be used in the WHERE clause
            tuple[9] = pgTarget.getTypeInfo().isSigned(typeOid) ? bf : bt;
            tuple[10] = bf; // false for now - must handle money
            tuple[11] = bf; // false - it isn't autoincrement
            tuple[13] = bZero; // min scale is zero
            // only numeric can supports a scale.
            tuple[14] = (typeOid == Oid.NUMERIC) ? Utils.encodeString("1000") : bZero;
            ...
            if ("int4".equals(typname)) {
                ...
                tuple1[0] = Utils.encodeString("serial");
                tuple1[11] = bt;
                v.add(new Tuple(tuple1));
            } else if ("int8".equals(typname)) {
                byte[] @Nullable [] tuple1 = tuple.clone();

                tuple1[0] = Utils.encodeString("bigserial");
                tuple1[11] = bt;
                v.add(new Tuple(tuple1));
            } else if ("int2".equals(typname) && pgTarget.haveMinimumServerVersion(ServerVersion.v9_2)) {
                byte[] @Nullable [] tuple1 = tuple.clone();

                tuple1[0] = Utils.encodeString("smallserial");
                tuple1[11] = bt;
                v.add(new Tuple(tuple1));
            }
    ...
    public ResultSet getIndexInfo(
         ...
         */
        String sql;
        if (pgTarget.haveMinimumServerVersion(ServerVersion.v8_3)) {
            sql = "SELECT NULL AS TABLE_CAT, n.nspname AS TABLE_SCHEM, "
                    ...
                    + (pgTarget.haveMinimumServerVersion(ServerVersion.v9_6) ? "  am.amname AS AM_NAME " : "  am.amcanorder AS AM_CANORDER ")
                    + "FROM pg_catalog.pg_class ct "
                    + "  JOIN pg_catalog.pg_namespace n ON (ct.relnamespace = n.oid) "
            ...
            sql = "SELECT "
                    ...
                    + (pgTarget.haveMinimumServerVersion(ServerVersion.v9_6)
                    ? "  CASE tmp.AM_NAME "
    ...
    public ResultSet getUDTs(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        TypeInfo typeInfo = pgTarget.getTypeInfo();
        ...
        if (pgTarget.getHideUnprivilegedObjects()
                && pgTarget.haveMinimumServerVersion(ServerVersion.v9_2)) {
            sql += " AND has_type_privilege(t.oid, 'USAGE')";
        }
    ...
    public ResultSet getClientInfoProperties() throws SQLException {
        ...
        if (pgTarget.haveMinimumServerVersion(ServerVersion.v9_0)) {
            ...
            tuple[0] = Utils.encodeString("ApplicationName");
            tuple[1] = Utils.encodeString(Integer.toString(getMaxNameLength()));
            tuple[2] = Utils.encodeString("");
            tuple[3] = Utils.encodeString("The name of the application currently utilizing the connection.");
            v.add(new Tuple(tuple));
        }
    ...
    public ResultSet getFunctions(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        boolean pgFuncResultExists = pgTarget.haveMinimumServerVersion(ServerVersion.v8_4);
    ...
    public ResultSet getFunctionColumns(@Nullable String catalog, @Nullable String schemaPattern,
        ...
        while (rs.next()) {
            ...
            if ("b".equals(returnTypeType) || "d".equals(returnTypeType) || "e".equals(returnTypeType)
                ...
                tuple[9] = null;
                tuple[10] = null;
                tuple[11] = Utils.encodeString(Integer.toString(java.sql.DatabaseMetaData.functionNullableUnknown));
                tuple[12] = null;
                tuple[14] = Utils.encodeString(Integer.toString(0));
            ...
            for (int i = 0; i < numArgs; i++) {
                ...
                tuple[5] =
                        Utils.encodeString(Integer.toString(pgTarget.getTypeInfo().getSQLType(argOid)));
                tuple[6] = Utils.encodeString(Objects.requireNonNull(pgTarget.getTypeInfo().getPGType(argOid)));
                tuple[7] = null;
                tuple[8] = null;
                tuple[9] = null;
                tuple[10] = null;
                tuple[11] =
                        Utils.encodeString(Integer.toString(DatabaseMetaData.functionNullableUnknown));
                tuple[12] = null;
                tuple[14] = Utils.encodeString(Integer.toString(i + 1));
                tuple[15] = isnullableUnknown;
                tuple[16] = specificName;
